...
export async function runBacktest(
  ...
  console.log(
    `[BACKTEST] Starting chunked simulation for ${symbol} using ${strategyName} (Equity: $${initialEquity})...`
  )
  ...
  for (let currentMs = startMs; currentMs < endMs; currentMs += CHUNK_MS) {
    ...
    for (let i = 0; i < ticks.length; i++) {
      ...
      // --- EXIT LOGIC ---
      if (position) {
        const { micro: regime, atr } = filter.getRegime(symbol, strategyConfig)
        const exit = checkTrailingStop(
          tick.price,
          position.entryPrice,
        ...
        if (!position) {
          ...
          if (history && history.length >= 60) {
            ...
            const {
              ...
              macro: macroRegime,
              atr
            } = filter.getRegime(symbol, strategyConfig)
            const rsi5m = calculateRSI(history, 14, 300)
            const scaledConfig = getScaledConfig(history, strategyConfig)

            const { micro: btcRegime, macroDiff: btcMacroDiff } = btcFilter.getRegime(
              'BTCUSDT',
              STRATEGIES.SNIPER
            )
