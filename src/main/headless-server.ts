...
async function startHeadless() {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('getSettings', async (callback) => {
      try { const s = await getSettings(); callback({ success: true, data: s }); }
      catch (err: any) { callback({ success: false, error: err.message }); }
    });

    socket.on('getWhitelist', async (callback) => {
      try { const w = await getWhitelist(); callback({ success: true, data: w }); }
      catch (err: any) { callback({ success: false, error: err.message }); }
    });
    ...
    socket.on('getFullGridState', async (...args) => {
      const callback = args.pop();
      try { const g = await getFullGridState(); if (typeof callback === 'function') callback({ success: true, data: g }); }
      catch (err: any) { callback({ success: false, error: err.message }); }
    });
    ...
    socket.on('runBacktest', async (symbol, start, end, shareAmount, gridStep, callback) => {
      try {
        ...
        const results = await runBacktest(symbol, start, end, shareAmount, gridStep, trailLevels, trailPct, (progress, interimResults) => {
