...
class RegimeFilter {
        ...
        if (volumeCapMult > 0 && this.windowVolSums[symbol][300] !== undefined) {
            ...
            if (avgVol > 0 && tickVolume > cap) {
                ...
                if (!this.lastCapLog || Date.now() - this.lastCapLog > 10000) {
                    console.log(`[VOLUME GUARD] ${symbol}: Clipping outlier trade (${tickVolume.toFixed(2)} -> ${cap.toFixed(2)}).`);
    ...
    getRegime(symbol, config = STRATEGIES.SNIPER) {
        ...
        if (distFromMacroLongMA >= 0 && distFromMacroLongMA <= bounceProximity && microSlope > 0 && macro !== 'BEAR') {
            isBouncing = true;
        }
...
const detectResistance = (history, currentPrice, config = STRATEGIES.SNIPER) => {
...
const shouldEnter = (symbol, regime, isBNB, btcRegime, whitelist = [], history = null, currentPrice = 0, btcMacroDiff = 0, config = STRATEGIES.SNIPER, macroRegime = 'UNKNOWN', isDecoupled = false, skipResistance = false, rsi = null, boxBounds = null, zScore = 0, autocorrelation = 0, obi = 0) => {
    ...
    if (boxBounds && boxBounds.low > 0 && boxBounds.high > 0) {
        ...
        if (boxRange > 0) {
            const pricePosition = (currentPrice - boxBounds.low) / boxRange;
            if (pricePosition > (config.BOX_BUY_ZONE || 0.20)) {
                return { allowed: false, reason: `PRICE_ABOVE_BOX_ZONE (${(pricePosition * 100).toFixed(1)}%)` };
            }
        }
    }
    ...
    if (symbol !== 'BTCUSDT' && !isDecoupled) {
        ...
        if (btcMacroDiff < config.BTC_STRENGTH_THRESHOLD) {
            return { allowed: false, reason: `BTC_TOO_WEAK (macroDiff: ${(btcMacroDiff * 100).toFixed(3)}%)` };
        }
    }
    ...
    if (!skipResistance && history && currentPrice > 0) {
        ...
        if (resistance !== null) {
            ...
            if (headroom < config.HEADROOM_REQUIRED) {
                return { allowed: false, reason: `RESISTANCE_TOO_CLOSE (headroom: ${(headroom * 100).toFixed(2)}%, level: ${resistance.toFixed(6)})` };
            }
        }
    }
...
const calculateATR = (history, candlePeriod = 900) => {
...
const calculateRSI = (history, period = 14, candlePeriod = 300) => {
...
const getScaledConfig = (symbol, history, config) => {
