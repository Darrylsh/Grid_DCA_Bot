import { pgTable, serial, text, doublePrecision, bigint, boolean, primaryKey, index, uniqueIndex } from 'drizzle-orm/pg-core';
...
export const trades = pgTable('trades', {
  ...
  timestamp: bigint('timestamp', { mode: 'number' }).default(sql`(extract(epoch from now()) * 1000)::bigint`),
  ...
  unq: uniqueIndex('unq_trades_event').on(t.symbol, t.side, t.price, t.quantity, t.timestamp, t.mode),
}));

export const whitelist = pgTable('whitelist', {
  symbol: text('symbol').primaryKey(),
  active: boolean('active').default(true),
  updatedAt: bigint('updated_at', { mode: 'number' }).default(sql`(extract(epoch from now()) * 1000)::bigint`),
});
...
export const gridLevels = pgTable('grid_levels', {
  ...
  createdAt: bigint('created_at', { mode: 'number' }).default(sql`(extract(epoch from now()) * 1000)::bigint`),
