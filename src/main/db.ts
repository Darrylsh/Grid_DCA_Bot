...
type GridLevelRow = typeof schema.gridLevels.$inferSelect
type CandleCacheRow = typeof schema.candleCache.$inferSelect
...
const initDb = async (): Promise<void> => {
  ...
  try {
    ...
    console.error('[DB] Initialization error:', e instanceof Error ? e.message : String(e))
  }
}
...
const getWhitelist = async (): Promise<string[]> => {
  const rows = await db.select().from(schema.whitelist).where(eq(schema.whitelist.active, true))
...
const getRecentTrades = async (mode = 'LIVE', limit = 50): Promise<TradeRow[]> => {
  return await db
    .select()
    .from(schema.trades)
...
const getTradesByTimeRange = async (
  mode = 'LIVE',
  startMs: number,
  endMs: number
): Promise<TradeRow[]> => {
  return await db
    ...
    .from(schema.trades)
    .where(
      and(eq(schema.trades.mode, mode), sql`timestamp >= ${startMs} AND timestamp <= ${endMs}`)
    )
...
// ---------------------------------------------------------------------------
const getGridLevels = async (symbol: string, mode = 'LIVE'): Promise<GridLevelRow[]> => {
  return await db
    .select()
...
const getAllActiveGridLevels = async (mode = 'LIVE'): Promise<GridLevelRow[]> => {
  return await db
    .select()
    .from(schema.gridLevels)
    .where(and(eq(schema.gridLevels.mode, mode), eq(schema.gridLevels.status, 'PENDING_SELL')))
}
...
const getCachedCandles = async (
  symbol: string,
  fromMs: number,
  toMs: number
): Promise<CandleCacheRow[]> => {
  return await db
    .select()
...
const getEarliestCachedCandle = async (symbol: string): Promise<number | null> => {
  const [row] = await db
    .select({ minTime: sql`MIN(open_time)` })
    .from(schema.candleCache)
...
const getLatestCachedCandle = async (symbol: string): Promise<number | null> => {
  const [row] = await db
    .select({ maxTime: sql`MAX(open_time)` })
    .from(schema.candleCache)
    .where(eq(schema.candleCache.symbol, symbol))
