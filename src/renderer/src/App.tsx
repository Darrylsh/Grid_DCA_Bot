      ...
      <div className="absolute top-4 right-6 flex gap-6 backdrop-blur-sm bg-slate-900/40 p-3 rounded-xl border border-slate-800/50">
        ...
        <div className="flex gap-4 items-center">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-widest text-[8px]">
              Entry
            </span>
          </div>
          <div className="flex items-center gap-1.5">
...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (window.api) {
      ...
      window.api.onTradeExecuted((data) => {
        ...
        setToast({
          message: `[${data.side}] ${stripUSDT(data.symbol)} @ $${data.price.toFixed(4)}`,
          type: 'success'
        })
      })
  ...
  const handleResetStats = async () => {
    if (
      window.confirm(
        `Are you sure you want to reset all ${tradingMode} stats? This will delete all trade history and cannot be undone.`
      )
    ) {
        ...
        <div className="flex items-center gap-3 mb-8">
          ...
          <div>
            ...
            <div className="flex items-center gap-2 text-xs text-slate-400">
              <span
                className={`w-2 h-2 rounded-full shadow-[0_0_8px_rgba(16,185,129,0.8)] transition-all duration-75 ${tickFlashing ? 'bg-emerald-400 scale-150' : 'bg-emerald-600 animate-pulse'}`}
              ></span>
              System Online
            </div>
            {botStartTime && (
        ...
        <div className="flex-1 flex flex-col min-h-0 bg-slate-800/20 rounded-xl p-2 border border-slate-700/30">
          ...
          <div className="flex gap-2 mb-6 px-1">
            ...
            <button
              ...
              className="bg-indigo-500 hover:bg-indigo-600 text-white px-3 rounded-lg transition-colors flex items-center justify-center font-bold"
            >
              +
            </button>
          </div>
          <ul className="flex flex-col gap-2 overflow-y-auto pr-1 custom-scrollbar pt-2">
            {whitelist.map((w) => {
              ...
              return (
                ...
                >
                  <span className="font-bold text-xs text-slate-200 tracking-wide min-w-[36px]">
                    {stripUSDT(w.symbol)}
                  </span>
        ...
        <header className="flex justify-between items-center mb-8">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">
              {activeTab === 'dashboard'
                ? 'Live Market Monitoring & Hybrid Execution'
                : activeTab === 'settings'
                  ? 'Configure trading parameters and maintenance'
                  : 'Simulate strategies against historical live data'}
            </p>
            {/* TODO:
            ...
            <div className="flex flex-col gap-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4 bg-slate-800/40 border border-slate-700/50 rounded-2xl p-6">
                ...
                <div className="flex items-center gap-2 mt-5">
                  ...
                  <label
                    htmlFor="btDecoupled"
                    className="text-[10px] font-bold text-slate-500 uppercase tracking-widest cursor-pointer select-none"
                  >
                    Ignore BTC Guard
                  </label>
                </div>
              ...
              {btResults && (
                <div className="animate-in fade-in slide-in-from-bottom-2 duration-500">
                  <div
                    className={`grid gap-4 mb-8 ${btResults.hasOpenPosition ? 'grid-cols-2 md:grid-cols-3 lg:grid-cols-6' : 'grid-cols-2 md:grid-cols-3 lg:grid-cols-5'}`}
                  >
                    <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-5">
                      ...
                      <h3
                        className={`text-2xl font-bold font-mono ${btResults.finalEquity >= (parseFloat(btEquity) || 0) ? 'text-emerald-400' : 'text-rose-400'}`}
                      >
                    ...
                    <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-5">
                      ...
                      <h3
                        className={`text-2xl font-bold font-mono ${(Number(btResults.totalPnl) || 0) >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}
                      >
                        {(Number(btResults.totalPnl) || 0) >= 0 ? '+' : ''}$
                        {(Number(btResults.totalPnl) || 0).toFixed(2)}
                      </h3>
                    </div>
                    <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-5">
                      <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-1">
                      ...
                      <h3
                        className={`text-2xl font-bold font-mono ${btResults.totalRoi >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}
                      >
                    ...
                    {btResults.hasOpenPosition && (
                      <div className="bg-amber-500/10 border border-amber-500/30 rounded-2xl p-5 relative overflow-hidden group">
                        ...
                        <h3
                          className={`text-2xl font-bold font-mono ${(Number(btResults.unrealizedPnl) || 0) >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}
                        >
                          {(Number(btResults.unrealizedPnl) || 0) >= 0 ? '+' : ''}$
                          {(Number(btResults.unrealizedPnl) || 0).toFixed(2)}
                        </h3>
                      </div>
                    )}
                    <div className="bg-slate-800/60 border border-slate-700/50 rounded-2xl p-5">
                  ...
                  <div className="mb-8">
                    <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-3 ml-1">
                      Market Context (Price vs. Signals)
                    </p>
                        ...
                        <tbody className="text-xs font-mono">
                          {btResults.trades.slice(0, 1000).map((t, i) => (
                            ...
                            >
                              ...
                              <td
                                className={`p-4 font-bold ${t.roi > 0 ? 'text-emerald-400' : t.roi < 0 ? 'text-rose-400' : 'text-slate-300'}`}
                              >
                                {t.side === 'SELL' ? (
                                  `${(t.roi * 100).toFixed(2)}%`
                                ) : i === btResults.trades.length - 1 &&
                                  btResults.hasOpenPosition ? (
                              ...
                              <td className="p-4 text-[10px] text-slate-500 italic uppercase underline decoration-indigo-500/30">
                                {t.reason || '-'}
                              </td>
                            </tr>
                          ))}
              ...
              <div className="grid grid-cols-4 gap-4 mt-10">
                {[
                  ...
                  },
                  {
                    ...
                    icon: Percent,
                    color: stats.avgRoi >= 0 ? 'text-blue-400' : 'text-rose-400',
                    bg: 'bg-blue-500/10'
                  },
                  {
                  ...
                  <div
                    key={i}
                    className="bg-slate-800/40 backdrop-blur-md border border-slate-700/50 rounded-2xl p-5 flex items-center gap-4 hover:bg-slate-800/60 transition-colors"
                  >
                    ...
                    <tbody className="divide-y divide-slate-700/50">
                      {Object.values(marketData).length === 0 ? (
                        <tr>
                          <td
                            colSpan={8}
                            className="p-8 text-center text-slate-500 flex flex-col items-center justify-center gap-3"
                          >
                        ...
                        Object.values(marketData)
                          ...
                          .map((row: any) => (
                            ...
                            >
                              ...
                              <td className="p-4">
                                <span
                                  className={`px-2 py-1 rounded text-[10px] font-semibold ${row.macroRegime === 'BULL' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-rose-500/10 text-rose-400'}`}
                                >
                                ...
                                {row.position ? (
                                  <div className="flex flex-col">
                                    <span className="font-bold text-slate-100">
                                      {row.position.quantity.toFixed(4)}
                                    </span>
                                    <span className="text-[10px] text-slate-500">
                                      @ ${row.position.entryPrice.toFixed(4)}
                                    </span>
                                  </div>
                                ) : (
                                  '-'
                              ...
                              <td className="p-4 font-mono">
                                <span
                                  className={`text-xs font-bold ${Math.abs(row.zScore) > 2 ? 'text-amber-400' : 'text-slate-400'}`}
                                >
                              ...
                              <td className="p-4 font-mono">
                                <span
                                  className={`text-xs font-bold ${row.autocorrelation > 0.2 ? 'text-emerald-400' : row.autocorrelation < -0.2 ? 'text-rose-400' : 'text-slate-400'}`}
                                >
                              ...
                              <td className="p-4 font-mono">
                                <span
                                  className={`text-xs font-bold ${row.obi > 0.6 ? 'text-emerald-400' : row.obi < 0.4 ? 'text-rose-400' : 'text-slate-400'}`}
                                >
                                  ...
                                  <div className="flex flex-col">
                                    <span
                                      className={`font-mono font-bold ${
                                        row.trailingStatus === 'TRAILING'
                                          ? 'text-indigo-400'
                                          : row.trailingStatus === 'MID_TRAIL'
                                            ? 'text-emerald-400'
                                            : 'text-amber-400'
                                      }`}
                                    >
                                      {((row.position.highWaterMark - row.position.entryPrice) /
                                        row.position.entryPrice) *
                                        100 >=
                                      0
                                        ? '+'
                                        : ''}
                              ...
                              <td className="p-4">
                                {row.position ? (
                                  <span
                                    className={`font-mono font-bold text-lg ${(row.currentPrice - row.position.entryPrice) / row.position.entryPrice >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}
                                  >
                                    {(
                                      ((row.currentPrice - row.position.entryPrice) /
                                        row.position.entryPrice) *
                                      100
                                    ).toFixed(2)}
                                    %
                              ...
                              <td className="p-4">
                                <span
                                  className={`font-mono font-bold ${row.rsi5m > 70 ? 'text-rose-400' : row.rsi5m < 30 ? 'text-emerald-400' : 'text-slate-300'}`}
                                >
                                  {row.rsi5m || '-'}
                                </span>
                              </td>
                                ...
                                <div className="flex bg-slate-900/50 p-1 rounded-lg border border-slate-700 gap-1 shrink-0">
                                  <button
                                    onClick={async () => {
                                      ...
                                      setToast({
                                        message: `Manual BUY order placed for ${stripUSDT(row.symbol)}`,
                                        type: 'success'
                                      })
                                    }}
                                  ...
                                  <button
                                    onClick={async () => {
                                      ...
                                      setToast({
                                        message: `Manual SELL order placed for ${stripUSDT(row.symbol)}`,
                                        type: 'success'
                                      })
                                    }}
                                  ...
                                  <button
                                    onClick={() =>
                                      window.api?.toggleBotManualMode(row.symbol, !row.manual)
                                    }
                    ...
                    logs.map((log, i) => (
                      ...
                      >
                        ...
                        <div className="flex justify-between w-full pr-2">
                          ...
                          >
                            {log.side
                              ? `${log.side} ${stripUSDT(log.symbol)} @ ${log.price}`
            ...
            <div className="max-w-2xl bg-slate-800/40 backdrop-blur-md border border-slate-700/50 rounded-2xl p-8 shadow-2xl">
              ...
              <div className="grid gap-8">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <p className="text-xs text-slate-500 mt-1 italic">
                    {tradingMode === 'LIVE'
                      ? '⚠️ WARNING: Real orders will be placed on your Binance account.'
                      : 'Simulation mode uses paper trading logic with your real balance.'}
                  </p>
                </div>
                ...
                <div className="grid grid-cols-2 gap-8">
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="text-sm font-semibold text-slate-400 uppercase tracking-wider">
                      Amount ({settings.capital_type === 'PERCENTAGE' ? '%' : '$'})
                    </label>
              ...
              <div className="mt-12 pt-8 border-t border-slate-800/60">
                ...
                <div className="bg-rose-500/5 border border-rose-500/20 rounded-2xl p-6 flex items-center justify-between shadow-xl shadow-rose-950/20">
                  <div>
                    <h4 className="font-bold text-rose-100 mb-1 caps">
                      Reset {tradingMode} Statistics
                    </h4>
                    <p className="text-xs text-slate-500">
                      Permanently delete all historical trade data and reset profit metrics for this
                      mode.
                    </p>
                  </div>
                  <button
